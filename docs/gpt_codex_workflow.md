...
本文档面向 ChatGPT Project 与人类协作者，是仓库维护的工作流说明；它不是 Project instructions，也不是 Codex 的仓库级执行规范。Project instructions 提供顶层硬约束和引用入口；本文档展开说明 GPT 如何把 Human + GPT 已同意的计划转化为 Codex execution prompt。Codex 的仓库级执行规范放在 `AGENTS.md`。可复用的 Codex 专项实操 workflow 放在 `.agents/skills/*/SKILL.md`，后续仍可继续扩展。
...
7. Codex skills 是例外：本仓库已有一批项目 skills；如果任务匹配某个 `.agents/skills/<skill-name>/SKILL.md`，GPT 应在 Codex prompt 中显式要求 Codex 使用对应 skill。
...
### 当前可用 Codex skills 的选择规则

本仓库当前已有以下项目 skills。GPT 生成 Codex prompt 时，应只在任务匹配专项 workflow 时调用；不要为了保险而调用 skill。任务跨多个关注点时可以同时调用多个 skills。如果没有可用 skill 匹配，写：`No Codex skill is required for this task.`

- `$fridge-system-closure-check`：任务影响端到端智能冰箱闭环时使用，例如 event detection、keyframes、YOLO diff、event JSON、inventory update、pending review、local service/API、manual confirmation 或 demo evidence。
- `$fridge-interface-contract-guard`：任务影响跨模块 contract 时使用，例如 JSON fields、event/stage2/final outputs、Module 1 -> 2 handoff、Module 2 -> 4 event fields、Module 4 inventory/pending structures、Module 5 service JSON、mini-program-facing API data、config keys、replay/session formats 或 schema compatibility。
- `$fridge-validation-evidence-pack`：任务需要产生或保留 testing、demo、report、paper、defense 证据时使用，例如 logs、event JSON、stage2/final artifacts、crops/overlays、API responses、build/test output、camera/board validation notes 或 evidence-level classification。
- `$fridge-embedded-portability-check`：任务影响端侧部署可移植性时使用，例如 C++ structure、inference backend、OpenCV/ONNX Runtime、CMake options、model runtime assumptions、SQLite/HTTP server、camera/video input、board deployment、external dependencies、cloud provider usage 或 edge deployment feasibility。

Skills 必须写在 Codex prompt 的 `Skills to use` 部分。Skills 是 prompt-level execution requirements for Codex，不是 human-facing configuration items。
...
Skills to use:
  No Codex skill is required for this task.
...
- 当前和未来 Codex skills 都不应重复 `AGENTS.md`；skill 应定义 recurring complex task type 的 specialized workflow，并让仓库级默认规则继续由 `AGENTS.md` 承担。
...
除非任务需要特殊阅读路径，prompt 不应重复 `AGENTS.md` 的完整默认阅读顺序；只列本任务相关的额外 docs/source files 即可。如果任务匹配仓库中当前可用或后续新增的 skill，Codex prompt 必须显式要求使用该 skill。
