...
本文档主要说明模块 1 / 视觉前段的 keyframe 和 event capture 流程，不是模块 2 的最新完整说明。模块 2 的 YOLO Runtime、ONNX Runtime / OpenCV DNN fallback、`YoloDiffAnalyzer`、session replay 和 mock/debug 路径，以 `cpp/module_2_yolo_analysis/README.md` 为准。
...
- `data/sessions/<session_id>/overlay.jpg`
...
- TODO：稳定模块 1 -> 模块 2 联调，确保 keyframe/event capture 输出能持续进入模块 2 的独立 YOLO 分析链路。
- TODO：将文件输入替换为真实摄像头视频流。
- TODO：补齐板端视频解码、嵌入式运行时适配和长时间运行验证。
