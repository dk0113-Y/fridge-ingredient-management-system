...
`cpp/cloud_classifier/` contains an independent C++ client skeleton for cloud-side fine-grained recognition.
...
It does not update inventory state or connect to the existing Python backend.
...
These settings stay outside C++ code so a future provider can be swapped in without hardcoding a specific vendor endpoint or model binding.
...
- The request payload can carry `request_id`, `model_name`, and optional `image_base64` based on config.
