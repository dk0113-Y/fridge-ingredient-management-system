...
struct RecognizerClientConfig {
    ...
    double llm_confidence_threshold = 0.75;
    std::string prompt_template =
        "Identify the ingredient from the image. "
        "Coarse class: {coarse_class}. Candidate labels: {candidate_labels}. "
        "Return JSON with keys name, category, confidence, reason, is_unknown, provider.";
};
